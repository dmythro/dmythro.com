I play a fanned-fret eight-string, and every time I ordered strings for it I was guessing. The scale length changes under every string, so a gauge that sits right under the low F# is a rubber band up at the high E — and every tension calculator I could find asked for one scale length and six strings.
...
Multi-scale is one extra input. Give the first and last string a scale length and the rest are interpolated, which is what a straight fan does anyway:
...
A six-string in D standard, a seven, and an eight-string fan all want different gauges to feel like the same instrument under the hand. Working that out meant a manufacturer's PDF, a unit-weight column, a calculator app, and a row of arithmetic per string — then the whole thing again the moment I reconsidered the tuning. The web calculators that could have saved me the arithmetic stopped at six strings and one scale length, which is precisely the case I didn't have.
...
**Use String Tension Pro if you're buying D'Addario and your neck isn't fanned.** It knows every product they make, down to the code on the packet. This one carries two brands' published tables and models everything else, so when your set is in their catalogue and your scale is a single number, their answer is the better answer.
