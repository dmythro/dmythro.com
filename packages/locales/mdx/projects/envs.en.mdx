...
**Not the macOS Keychain.** The Keychain is the obvious place for this on a Mac, and it can't do it: it cannot create iCloud-synchronised items programmatically — still true as of macOS 26 — and synced items run into roughly a 4 KB ceiling, which a real `.env` exceeds. A synced folder is a plain filesystem with no size limit that also works on Linux.

**Encrypted even though the sync service encrypts.** iCloud with Advanced Data Protection is end-to-end encrypted already, so this layer is redundant on paper. I want it anyway: it means the sync provider is not part of my threat model, and anyone who reaches that folder — a support session, a shared Mac, a future me being careless with permissions — sees opaque `.age` blobs. Defence in depth is cheap when it costs one `age` invocation.
