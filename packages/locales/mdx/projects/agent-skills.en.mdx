...
`agent-skills` is those paragraphs extracted into installable modules, so a fix lands once and every agent picks it up.
...
Skills that drive a CLI ship a `references/allowlist.md` with tiered auto-approval patterns for Claude Code's `settings.json` or an OpenCode config — the read-only commands that are safe to approve once instead of every time. `bun info`, `gh pr view`, `glab ci status`, `coderabbit doctor` and friends.
...
**Distribution is somebody else's problem.** [skills.sh](https://skills.sh) already handles installing, updating and locating skills per agent, so this repo is just well-structured markdown. If the format outlives the installer, the content is still fine.
